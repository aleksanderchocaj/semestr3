namespace _5_project_wyklad_2
{
    class Car
    {
        ...
        public void ShowCar()
        {
            Console.WriteLine($"Marka: {this.Brand}, model: {this.Model}\nPojemność: {this.Engine.Capacity}, " +
                $"ilość benzyny: {this.Engine.FuelAmount}, pojemność zbiornika: {this.Engine.fuelTankCapacity}");
        }
    }
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Podaj podaj pojemność zbiornika na paliwo (w litrach) : ");
            ...
            while (true) 
            {
                ...
                switch (choice)
                {
                    case 1: Console.WriteLine("Podaj dystans do przejechania (w kilometrach): "); 
                    int distance=int.Parse(Console.ReadLine());
                    c.Drive(distance);
                    break;
                    case 2: Console.WriteLine("Podaj ilosc paliwa do zatankowania (w litrach): ");
                    double fuelToAdd = double.Parse(Console.ReadLine());
                        c.Refuel(fuelToAdd);
                    break;
                    case 3: Console.WriteLine($"Ilość paliwa: {c.Engine.FuelAmount}");
                    break;
                    case 4: c.ShowCar();
                    break;
            ...
            Console.WriteLine($"Marka: {c.Brand}, model: {c.Model}\nPojemność: {c.Engine.Capacity}, " +
                $"ilość benzyny: {c.Engine.FuelAmount}, pojemność zbiornika: {c.Engine.fuelTankCapacity}");
